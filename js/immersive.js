...
const deviceLaunch=document.createElement('button');deviceLaunch.className='immersive-device-launch';deviceLaunch.type='button';deviceLaunch.textContent='ENTER SPATIAL MODE ↗';deviceLaunch.setAttribute('aria-label','Enter immersive spatial mode');document.body.appendChild(deviceLaunch);deviceLaunch.addEventListener('click',openSpace);
async function detectXR(){try{const xr=navigator.xr;if(!xr)return;const vr=await xr.isSessionSupported('immersive-vr').catch(()=>false);const ar=await xr.isSessionSupported('immersive-ar').catch(()=>false);if(vr||ar){deviceLaunch.classList.add('visible');deviceLaunch.textContent=vr?'ENTER VR / SPATIAL ↗':'ENTER MR / SPATIAL ↗';deviceLaunch.setAttribute('aria-label',vr?'Enter VR immersive spatial mode':'Enter MR immersive spatial mode');if(!window.matchMedia('(pointer:coarse)').matches) return;setTimeout(()=>{if(!shell.classList.contains('open'))openSpace();},350);}}catch{}}
