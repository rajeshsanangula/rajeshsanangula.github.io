...
// XR LAUNCHER FAILSAFE v7.2 — intentionally independent of the spatial module UI.
(function(){
  const mount=()=>{
    if(document.getElementById('rajesh-xr-failsafe')) return;
    const b=document.createElement('button');
    b.id='rajesh-xr-failsafe'; b.type='button'; b.textContent='ENTER IMMERSIVE EXPERIENCE ↗';
    Object.assign(b.style,{position:'fixed',right:'18px',bottom:'18px',zIndex:'2147483647',display:'block',padding:'13px 16px',border:'1px solid rgba(111,226,255,.75)',borderRadius:'0',background:'rgba(4,9,22,.94)',color:'#effbff',font:'600 10px/1.2 Arial,sans-serif',letterSpacing:'.14em',textTransform:'uppercase',cursor:'pointer',boxShadow:'0 10px 40px rgba(0,0,0,.45)',backdropFilter:'blur(18px)'});
    b.onclick=async()=>{
      try { if (navigator.xr && await navigator.xr.isSessionSupported('immersive-vr') && window.__rajeshStartXR) { await window.__rajeshStartXR('vr'); return; } } catch(e) {}
      if(window.__rajeshOpenImmersive) window.__rajeshOpenImmersive();
    };
...
window.__rajeshStartXR = startXR;
function openSpace() { mode = 'space'; ready = false; onboarding.classList.remove('hide'); shell.classList.add('open'); document.body.style.overflow = 'hidden'; shell.classList.remove('xr-ar'); statusEl.textContent = 'SPATIAL ENVIRONMENT · READY'; resize(); if (!raf)
    frame(); }
window.__rajeshOpenImmersive = openSpace;
...
const deviceLaunch=document.createElement('button');deviceLaunch.className='immersive-device-launch visible';deviceLaunch.type='button';deviceLaunch.textContent='ENTER IMMERSIVE EXPERIENCE ↗';deviceLaunch.setAttribute('aria-label','Enter immersive experience');document.body.appendChild(deviceLaunch);deviceLaunch.addEventListener('click',async()=>{try{if(navigator.xr && await navigator.xr.isSessionSupported('immersive-vr')){await startXR('vr');return;}}catch{} openSpace();});
async function detectXR(){try{const xr=navigator.xr;if(!xr)return;const vr=await xr.isSessionSupported('immersive-vr').catch(()=>false);const ar=await xr.isSessionSupported('immersive-ar').catch(()=>false);if(vr||ar){deviceLaunch.textContent=vr?'ENTER VR / SPATIAL ↗':'ENTER MR / SPATIAL ↗';deviceLaunch.setAttribute('aria-label',vr?'Enter VR immersive spatial mode':'Enter MR immersive spatial mode');}}catch{}}
