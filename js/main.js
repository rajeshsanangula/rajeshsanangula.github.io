...
const certs = [
    ['Professional Integration Developer Certification', 'Valid: July 21, 2025 → July 21, 2027', './assets/certifications/cert-1.jpg', 'https://verify.skilljar.com/c/ontgdgi4ru4z'],
    ['Associate EDI for X12 Certification - Legacy', 'Valid: October 29, 2022 → October 29, 2027', './assets/certifications/cert-2.jpg', 'https://verify.skilljar.com/c/pzxrg4t5pger'],
    ['Associate Administrator Certification', 'Valid: August 29, 2025 → August 29, 2030', './assets/certifications/cert-3.jpg', 'https://verify.skilljar.com/c/knbyrg9owqy8'],
    ['Associate Integration Architect Certification', 'Valid: August 30, 2025 → August 30, 2030', './assets/certifications/cert-4.jpg', 'https://verify.skilljar.com/c/84oub4oiavsw'],
    ['Professional Linux Operational Administrator Certification', 'Valid: September 11, 2025 → September 11, 2027', './assets/certifications/cert-5.jpg', 'https://verify.skilljar.com/c/3r2j8aojq92n'],
    ['Professional API Design Certification', 'Valid: August 26, 2026 → August 26, 2028', './assets/certifications/cert-7.jpg', 'https://verify.skilljar.com/c/5rxh5tssdra6'],
    ['Professional API Management Certification', 'Valid: August 26, 2026 → August 26, 2028', './assets/certifications/cert-8.jpg', 'https://verify.skilljar.com/c/zhvnnsawnyi2'],
    ['Associate Integration Developer Certification - Legacy', 'Valid: November 17, 2022 → November 17, 2027', './assets/certifications/cert-9.jpg', 'https://verify.skilljar.com/c/bfa557er6aok']
];
const awards = [
...
if (constellation) {
    ...
    constellation.querySelectorAll('.constellation-icon img').forEach(img => { img.addEventListener('error', () => { img.hidden = true; const fallback = img.nextElementSibling; if (fallback)
        fallback.hidden = false; }, { once: true }); });
...
document.querySelectorAll('.cert-verify').forEach(el => {
    const go = () => { const url = el.dataset.verify; if (url)
...
cvViewer?.addEventListener('click', e => { if (e.target === cvViewer || e.target.classList.contains('pdf-close'))
    closeCv(); });
const cvPages = document.querySelector('#cv-pages');
const cvPageStatus = document.querySelector('#cv-page-status');
if (cvPages && cvPageStatus) {
    const pages = Array.from(cvPages.querySelectorAll('img'));
    const updateCvPage = () => {
        if (!pages.length)
            return;
        const viewport = cvPages.getBoundingClientRect();
        const center = viewport.top + viewport.height * 0.38;
        ...
        cvPageStatus.textContent = `PAGE ${String(best + 1).padStart(2, '0')}/${String(pages.length).padStart(2, '0')}`;
    };
    cvPages.addEventListener('scroll', () => requestAnimationFrame(updateCvPage), { passive: true });
    window.addEventListener('resize', updateCvPage);
    updateCvPage();
}
